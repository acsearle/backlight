...
cBound coCone::Bound(Array_Lights & L_S_L)
{
    ...
    m_bounded_by.o() = Transformation.Vector(2) * (m_Radius_1 + m_Radius_2) + Transformation.Vector(3);
...
double coCone::Interior(lift::vector<double, 3> o)
{
    ...
    if(o[0] > 0) {
        if(o[0] < 1) {
            if( (o[1] *o[1] + o[2] *o[2]) < ((1-o[0]) *m_Radius_1 + o[0] *m_Radius_2) * ((1-o[0]) *m_Radius_1 + o[0] *m_Radius_2) ) {
                f = 1;
            }
        }
    }
...
ciObject coCone::Intersection(cRay &RInitial, double t_min, double t_max, poObject poParent) {
    ciObject IBest;
...
pToken  coCone::Load(pToken  T) 
{
    ...
    EXPECTED(T,LEFT_BRACE);
        ...
        if( (n3 == lift::make_vector<double>( 0, 0, 0 )) || (( m_Radius_1 < EPSILON ) && (m_Radius_2 == EPSILON)) )
        {
        ...
        Transformation.Vector(3) = vb - lift::make_vector<double>(0,0,scale*m_Radius_1);
...
lift::vector<double, 3> coCone::Normal(lift::vector<double, 3> o)
{
    ...
    n = m_matGeometry.TransformLocation(o) - lift::make_vector<double>( 0, 0, 0.5 ) *(m_Radius_1+m_Radius_2);            
    
    //on cap 1?
    if( fabs(n[2] + (m_Radius_2- m_Radius_1)/2) < EPSILON )
        n[2] = 1/EPSILON;  
    //on cap 2?
    ...
    {
        //on the cone
        n  = lift::make_vector<double>(n[0],n[1],0.0); Normalise(n,1/sqrt(2.0));
