...
cBound coCylinder::Bound(Array_Lights & L_S_L)
{
    ...
    m_bounded_by.r() =
      std::max(
      std::max(
          abs(Transformation.Vector(1) * 0.5 + Transformation.Vector(0) + Transformation.Vector(2)),
          abs(Transformation.Vector(1) * 0.5 + Transformation.Vector(0) - Transformation.Vector(2))
        ),
        std::max(
          abs(Transformation.Vector(1) * 0.5 - Transformation.Vector(0) + Transformation.Vector(2)),
...
ciObject coCylinder::Intersection(cRay &RInitial, double t_min, double t_max, poObject poParent) {
    ciObject IBest;
...
lift::vector<double, 3> coCylinder::Normal(lift::vector<double, 3> o) 
{
    ...
    n = m_matGeometry.TransformLocation(o) - lift::make_vector<double>( 0, 0.5, 0.0 );
