...
coObject::~coObject() {
    if( m_Surface != NULL && m_Surface != Default_Texture )
        delete m_Surface;
...
bool coObject::Bound_Intersection( cRay & RInitial, double t_min, double t_max, poObject poParent )
{
    ...
    // object implementations
        ...
        if(m_bounded_by.r() > 0)
        {
          // coObject is bounded
            ...
            b = inner_product(RInitial.Direction(), (RInitial.Origin().p() - m_bounded_by.o())) * 2.0;
            c = inner_product(RInitial.Origin().p() - m_bounded_by.o()) - sqr(m_bounded_by.r());
    ...
    return true; //object is not bounded, so all rays potentially intersect with it
};
...
ciObject coObject::Intersection(cRay &RInitial, double t_min, double t_max, poObject poParent)
{
    ...
    IBest.t() = t_max;
            ...
            printf("How on earth could an unregistered shape reach the coObject::Intersection routine ?! (THIS IS IMPOSSIBLE)\n");
            assert(false);
