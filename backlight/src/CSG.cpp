...
void coCSG::init() { m_operation = CSG_NONE; };
...
void coCSG::Octree() {
      ...
      switch (i) {
        case 0:
          if (((bound.o() - m_bounded_by.o())[0] < 0) &&
            ...
            transfer_object(static_cast<coCSG*>(Hierarchy[i])->m_Object_List, k,
                            m_Object_List, j);
            k++;
          } else
            j++;
  ...
  for (i = 0; i < m_Object_List.size();)
    transfer_object(static_cast<coCSG*>(Hierarchy[8])->m_Object_List, i,
                    m_Object_List, i);
...
ciObject coCSG::Intersection(cRay& RInitial, double t_min, double t_max,
                             poObject poParent) {
  ...
  switch (m_operation) {
    case CSG_UNION:
      ...
      while (i < m_Object_List.size()) {
        if (m_Object_List[i]->Bound_Intersection(RInitial, t_min, t_max,
                                                 poParent)) {
          ICurrent = m_Object_List[i]->Intersection(RInitial, IBest.t(), t_max,
                                                    poParent);

          if (m_Object_List[i]->bounded_by().r() > 0) i = i;

          // Now, if it has been updated, it means that YES, it is
          // a better choice than IBest, so

          if (ICurrent.From_Object() != NULL)
            if (ICurrent.t() < 0 && ICurrent.t() > IBest.t()) IBest = ICurrent;
        };
    ...
    case CSG_MERGE:
      ...
      i = 0;
      while (i < m_Object_List.size()) {
        if (m_Object_List[i]->Bound_Intersection(RInitial, FAR_VAL, NEAR_VAL,
                                                 poParent)) {
          ICurrent = m_Object_List[i]->Intersection(RInitial, IBest.t(), lt_max,
                                                    lpoParent);
...
pToken coCSG::Load(pToken T) {
  ...
  // determine the number of sub-objects to allocate the array
  {
    ...
    if (m_Object_List.size() == 1) {
      if (Globals.Verbose() >= 1) {
        printf(
            "Warning: Should be more than 1 object in CSG in %s at line %li\n",
            T->Filename(), T->Line());
      }
    }
