...
void alloc_by_block(unsigned long allocations, DT* buffer) {
  ...
  for (i = 0, j = 0; i < n_allocs; i++) {
    t = new DT[min_alloc];
    for (k = 0; k < min_alloc; j++, k++) buffer[j] = t[k];
  }
  t = new DT[extra_alloc];
  for (k = 0; k < extra_alloc; j++, k++) buffer[j] = t[k];
}
...
template<class DT2, class DT1>
tSafePointer<DT2> SP_CAST2(tSafePointer<DT1> & sp1) { return
tSafePointer<DT2>((DT2 *)(DT1 *)sp1 ); };
*/
template <class DT2, class DT1>
DT2 SP_CAST1(DT1& sp1) {
  return (DT2)sp1;
};

template <class DT2, class DT1>
DT2* SP_CAST2(DT1& sp1) {
  return (DT2*)sp1;
};
...
class tSafeArray_Static {
 private:
  ...
  void init(DT d = m_initializer) {
    for (unsigned long i = 0; i < DIMENSIONS; i++) m_data[i] = d;
  };
 ...
 public:
  ...
  DT& operator[](long i) {
    if (i >= DIMENSIONS)
      throw "tSafeArray_Static::[] Read past end of array error.";
...
class tSafeArray_Dynamic {
 private:
  void init(void) {
    for (unsigned long i = 0; i < m_allocated; i++) m_data[i] = m_initializer;
  };
 ...
 public:
  // CTORS
  tSafeArray_Dynamic() : m_initializer() {
    m_data = NULL;
    allocate(0);
  };
  tSafeArray_Dynamic(long i) : m_initializer() {
    m_data = NULL;
    allocate(i);
  };
  tSafeArray_Dynamic(long i, DT& in) : m_initializer(in) {
    m_data = NULL;
    allocate(i);
    init();
  };
  ...
  void Initialize(DT& d) {
    ...
    for (long i = 0; i < m_allocated; i++) *m_data[i] = d;
  };
  ...
  void reallocate_another(unsigned long i) { reallocate(m_first_free + i); };
  ...
  tSafeArray_Dynamic<DT>& operator=(tSafeArray_Static<DT, N>& a) {
    long i;
  ...
  DT& operator[](unsigned long i) {
    if (i < m_allocated)
      ...
      throw "Read past end of array error.";
      return m_data[0];
    };
  }
  ...
  void qsort() {
    QSORT((void*)m_data, (unsigned long)m_first_free, sizeof(DT*),
          DT::operator>);
  };
};
...
class tStack_Pointer_Static {
 ...
 public:
  ...
  void init(void) {
    for (long i = 0; i < STACK_DEPTH; i++) buffer[i] = NULL;
    m_first_free = 0;
...
DT* tStack_Pointer_Static<DT, STACK_DEPTH>::pop_heap(void) {
  ...
  if (m_first_heap < STACK_DEPTH) {
    ...
    m_data[m_first_heap] = NULL m_first_heap++;
  } else
    t = NULL;
...
void tStack_Pointer_Static<DT, STACK_DEPTH>::push(DT* item) {
  if (m_first_free >= STACK_DEPTH)
    throw "tStack_Pointer_Static::push : Stack overflow error.";
...
template <class DT, unsigned long STACK_DEPTH>
class tStack_Static : public tStack_Pointer_Static<DT, STACK_DEPTH> {
 protected:
 ...
 public:
   ...
   *
          DT* create(){
              ...
              if( buffer[m_first_free] != NULL ) {  //assume the simplest case
   first

                  //unlink this element (will probably be relinked very soon,
   but ...)
...
class tStack_Pointer_Dynamic {
 protected:
  ...
  void reallocate_another(unsigned long i) { reallocate(m_allocated + i); }
...
void tStack_Pointer_Dynamic<DT>::move_heap(unsigned long i, unsigned long j) {
  ...
  for (k = i, m = j; k > m_first_heap; k--, m--) buffer[m] = buffer[k];
...
template <class DT>
class tStack_Dynamic : public tStack_Pointer_Dynamic<DT> {
 protected:
  void insert(unsigned long i, DT* item);
  void destroy(void);
 ...
 public:
  // CTORS
  tStack_Dynamic() : tStack_Pointer_Dynamic<DT>(MIN_ALLOCATION) { init(); };
  tStack_Dynamic(unsigned long i) : tStack_Pointer_Dynamic<DT>(i) { init(i); };
...
unsigned long tQueue_Static<DT, QUEUE_LENGTH>::Size() {
  if (next > first)
    ...
    return (next + QUEUE_LENGTH - first);
};
...
template <class DT, unsigned long QUEUE_LENGTH>
class tQueue_Static_Prioritized : tQueue_Static<DT, QUEUE_LENGTH> {
 protected:
  // ACCESSORS
 ...
 public:
  ...
  // OPERATORS
  virtual bool operator<(tQueue_Static_Prioritized<DT, QUEUE_LENGTH>) {
    return true;
  };
};
...
template <class DT, unsigned long QUEUE_LENGTH>
void tQueue_Static_Prioritized<DT, QUEUE_LENGTH>::insert(unsigned long i,
                                                         DT d) {
  long j;
...
unsigned long tQueue_Dynamic<DT>::Size() {
  if (next > first)
    ...
    return (next + queue.Length() - first);
};
...
template <class DT>
class tQueue_Dynamic_Prioritized : public tQueue_Dynamic<DT> {
 protected:
  // ACCESSORS
  void insert(unsigned long i, DT d);
  DT remove(unsigned long i);
