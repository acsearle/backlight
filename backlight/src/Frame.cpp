...
ciFrame cFrame::Intersection(cRay &RInitial, poObject poParent) {
  ...
  while (i < m_Object_List.size()) {
    ...
    // FIXUP: There is a quick and dirty solution here, with t_max nonzero! This
    // means
    // that surfaces not belonging to the parent surface (should it exist) that
    // lie
    // within this difference will be clipped back, possibly resulting in shadow
    // speckle
    // in rare cases - but believe me, it's the lesser of two evils, because
    // otherwise
    // all coincident surfaces speckle!

    // Using a 1e-6 (a micron) as surface clip - this is about the wavelength of
    // light
    ...
    if (m_Object_List[i]->Bound_Intersection(RInitial, FAR_VAL, NEAR_VAL,
                                             poParent))
      ICurrent =
          m_Object_List[i]->Intersection(RInitial, FAR_VAL, NEAR_VAL, poParent);
  ...
  while (T->ID() != ID_RIGHT_BRACE) {
    ...
    switch (T->ID()) {
      ...
      case ID_camera:
        ...
        //          case ID_RIGHT_BRACE:    // Only possible for first interior
        //          token
        //              T = T->Next();
        //              printf("Empty frame definition in %s at line %li, please
        //              remove\n", T->Filename(), T->Line());
        //              assert(false);
        //              break;
    }
