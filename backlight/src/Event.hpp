...
class cEvent : public RefCount {
 ...
 public:
  cEvent() { m_t = 0; };
  cEvent(double _x, double _y, double _z, double _t)
      : m_pos(lift::make_vector(_x, _y, _z)), m_t(_t){};
  ...
  cEvent LorentzTransform(lift::vector<double, 3>& v,
                          lift::vector<double, 3>& u);

  // inner product
  //    _inline double operator * (cEvent e) { return m_pos * e.m_pos - m_t *
  //    e.m_t; };

  _inline cEvent operator+(cEvent e) {
    return cEvent(m_pos + e.m_pos, m_t + e.m_t);
  };
  _inline cEvent operator-(cEvent e) {
    return cEvent(m_pos - e.m_pos, m_t - e.m_t);
  };
  //  _inline cEvent operator & (cEvent e) { return cEvent( m_pos & e.m_pos, m_t
  //  * e.m_t); };
  //    _inline cEvent operator % (cEvent e) { return cEvent( m_pos % e.m_pos,
  //    m_t / e.m_t); };
};
