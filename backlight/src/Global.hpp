...
class RefCount {
};  // dummy class; replace with intrusive reference counting or shared_ptr?
...
class cSettings_Global : public RefCount {
 ...
 protected:
  double m_Verbose, m_Antialias, m_Sampling_Method, m_Antialias_Threshold,
      m_Antialias_Depth, m_adc_bailout, m_assumed_gamma, m_max_trace_level,
      m_Relativity, m_Observation, m_Doppler, m_Intensity, m_Display,
      m_Window_l, m_Window_r, m_Window_t, m_Window_b, m_Window_Doppler,
      m_Window_Intensity, m_NW_Doppler, m_NW_Intensity, m_HUD;
  long m_Tokens, m_MemoryUsed, m_PrimitivesUsed, m_Continue, m_Recursion;
...
#define REFINE_CHOISE(a, b, c, d) \
  if ((b) <= 0 && (b) > (a))      \
    if (BetweenQ((b), (c), (d))) (a) = (b)
