...
void coBox::Initialize() { coObject::Initialize(); };
...
cBound coBox::Bound(Array_Lights& L_S_L) {
  ...
  m_bounded_by.o() = Transformation.Vector(0) * 0.5 +
                     Transformation.Vector(1) * 0.5 +
                     Transformation.Vector(2) * 0.5 + Transformation.Vector(3);

  // Now, test to find longest diagonal

  m_bounded_by.r() =
      max(max(abs(Transformation.Vector(0) + Transformation.Vector(1) +
                  Transformation.Vector(2)) /
                  2,
              abs(Transformation.Vector(0) + Transformation.Vector(1) -
                  Transformation.Vector(2)) /
                  2),
          max(abs(Transformation.Vector(0) - Transformation.Vector(1) +
                  Transformation.Vector(2)) /
                  2,
              abs(Transformation.Vector(0) - Transformation.Vector(1) -
                  Transformation.Vector(2)) /
...
ciObject coBox::Intersection(cRay& RInitial, double t_min, double t_max,
                             poObject poParent) {
  ciObject IBest;
  ...
  if (d == lift::make_vector<double>(0, 0, 0)) {
    ...
    printf(
        "coObject::Intersection [box] Null ray detected (THIS IS "
        "IMPOSSIBLE)\n");
...
pToken coBox::Load(pToken T) {
  ...
  Transformation = cMatrix(v2[0] - v1[0], 0, 0, 0, v2[1] - v1[1], 0, 0, 0,
                           v2[2] - v1[2], v1);
...
lift::vector<double, 3> coBox::Normal(lift::vector<double, 3> o) {
  ...
  n = m_matGeometry.TransformLocation(o) -
      lift::make_vector<double>(0.5, 0.5, 0.5);
