...
cFColour cScene::GetColour(ciFrame &IBest) {
  ...
  if (p_object == NULL) {
    Temp_Ray = Eye_Ray.LorentzTransform(IBest.From_Frame()->Velocity(),
                                        SkySphereVelocity);
    object_col =
        SkySphereSurface
            .GetPigment(
                cEvent(Temp_Ray.Direction() * -1, Temp_Ray.Origin().t()))
            .Doppler(Temp_Ray.doppler() * SkySphereSurface.P().doppler()) *
        Temp_Ray.intensity() * SkySphereSurface.P().intensity();
  ...
  object_col = p_object->GetPigment(*p_event).Doppler(Eye_Ray.doppler() *
                                                      p_pigment->doppler());

  // Ambient light...

  c = object_col * (p_finish->ambient().Doppler(Eye_Ray.doppler()) *
                    Eye_Ray.intensity() * p_pigment->intensity());
  ...
  if ((p_finish->reflection().r() != 0) || (p_finish->reflection().g() != 0) ||
      (p_finish->reflection().b() != 0)) {
    ...
    Reflected_Ray.Direction() =
        Eye_Ray.Direction() - n * 2.0 * inner_product(n, Eye_Ray.Direction());
    ...
    c +=
        GetColour(ICurrent) * p_finish->reflection().Doppler(Eye_Ray.doppler());
  }
  ...
  while (i < m_Number_Frames) {
    ...
    Current_Hit = p_event->LorentzTransform(IBest.From_Frame()->Velocity(),
                                            m_Frame_List[i]->Velocity());
    ...
    while (j < m_Frame_List[i]->Number_Lights()) {
      ...
      Light_Ray = Light_Ray.LorentzTransform(m_Frame_List[i]->Velocity(),
                                             IBest.From_Frame()->Velocity());
      Light_Ray.intensity() = Eye_Ray.intensity();
      Light_Ray.doppler() = Eye_Ray.doppler();
      Light_Ray = Light_Ray.LorentzTransform(IBest.From_Frame()->Velocity(),
                                             m_Frame_List[i]->Velocity());
      ...
      if (m_Frame_List[i]->Light_List(j)->no_shadow()) {
        ...
        while ((p_tobject != NULL) &&
               ((Current_Hit.t() - ICurrent.Hit().t()) < abs(d))) {
          if (p_tobject->no_shadow()) {
            // just call GetPigment() and let that function sort out whether to
            // use a quick colour
            Shadow_Hit = ICurrent.Hit().LorentzTransform(
                m_Frame_List[i]->Velocity(), ICurrent.From_Frame()->Velocity());
            surface_col =
                p_tobject->GetPigment(Shadow_Hit)
                    .Doppler(Light_Ray.doppler() * p_tpigment->doppler()) *
                Light_Ray.intensity();

            transmitted_col *= ((cFColour(1, 1, 1, 0, 0) * surface_col.t()) +
                                (surface_col * surface_col.f()));

            if ((transmitted_col.r() <= 0) && (transmitted_col.g() <= 0) &&
                (transmitted_col.b() <= 0)) {
      ...
      if ((ICurrent.From_Object() == NULL) ||
        ...
        // Light_Ray.Direction() =
        // Light_Ray.Direction().LorentzTransform(m_Frame_List[i]->Velocity(),
        // IBest.From_Frame()->Velocity());

        ld = Light_Ray.Direction() * -1.0;
        Normalise(ld, 1);
        ed = Eye_Ray.Direction();
        Normalise(ed, 1);
        rd = (ed - n * (2 * inner_product(n, ed))) * -1;
        Normalise(rd, 1);
        dd = (n + rd);
        Normalise(dd, 1);
        ...
        if (inner_product(ld, n) > 0) {
          ...
          light_col = m_Frame_List[i]
                          ->Light_List(j)
                          ->GetPigment(cEvent(
                              o[0] + (d[0] / abs(d)), o[1] + (d[1] / abs(d)),
                              o[2] + (d[2] / abs(d)), Current_Hit.t() - abs(d)))
                          .Doppler(Light_Ray.doppler());

          // FIXUP: for intensity...
          Light_Ray.intensity() *=
              m_Frame_List[i]->Light_List(j)->Surface()->P().intensity();
          ...
          if (m_Frame_List[i]->Light_List(j)->fade_power() != 0) {
            Light_Ray.intensity() *=
                ...
                (1.0 +
                 pow(abs(d) /
                         ((oLight *)m_Frame_List[i]->Light_List(j))
                             ->fade_distance(),
                     ((oLight *)m_Frame_List[i]->Light_List(j))->fade_power()));
          }
          ...
          if (p_finish->diffuse() != 0) {
            if (p_finish->brilliance() == 1) {
              c += transmitted_col * (object_col * light_col) *
                   inner_product(n, ld) *
                   ((1 + inner_product(ld, dd) * sqrt(2.)) / (1 + sqrt(2.))) *
                   p_finish->diffuse() * (1 - object_col.t() - object_col.f());
            } else {
              c += transmitted_col * (object_col * light_col) *
                   inner_product(n, ld) *
                   pow((1 + inner_product(ld, dd) * sqrt(2.)) / (1 + sqrt(2.)),
                       p_finish->brilliance()) *
                   p_finish->diffuse() * (1 - object_col.t() - object_col.f());
            }
          }
          ...
          if (p_finish->specular()) {
            if (p_finish->metallic()) {
              ...
              c += transmitted_col * object_col * inner_product(n, ld) *
                   pow((1 + inner_product(ld, rd)) / 2,
                       1 / p_finish->roughness()) *
                   p_finish->specular();
            } else {
              c += transmitted_col * light_col * inner_product(n, ld) *
                   pow((1 + inner_product(ld, rd)) / 2,
                       1 / p_finish->roughness()) *
                   p_finish->specular();
            }
...
ciFrame cScene::CastRay(cRay &RInitial, lift::vector<double, 3> &v,
                        poObject poParent) {
  cRay Current_Ray;
  ...
  while (i < m_Number_Frames) {
    // Test for any objects in the frame before we start chewing m_up precious
    // time!
    // if(m_Frame_List[i]->Number_Objects() > 0)
    {
      ...
      Current_Hit =
          ICurrent.Hit().LorentzTransform(m_Frame_List[i]->Velocity(), v);
...
cFColour cScene::Antialias(cFColour c00, cFColour c02, cFColour c20,
                           cFColour c22, double u_0, double v0_, double u_2,
                           double v2_, pCamera C) {
  ...
  if ((fabs(c22.r() - c02.r()) + fabs(c22.g() - c02.g()) +
       fabs(c22.b() - c02.b())) < Globals.Antialias_Threshold()) {
    if ((fabs(c22.r() - c20.r()) + fabs(c22.g() - c20.g()) +
         fabs(c22.b() - c20.b())) < Globals.Antialias_Threshold()) {
      if ((fabs(c00.r() - c02.r()) + fabs(c00.g() - c02.g()) +
           fabs(c00.b() - c02.b())) < Globals.Antialias_Threshold()) {
        if ((fabs(c00.r() - c20.r()) + fabs(c00.g() - c20.g()) +
             fabs(c00.b() - c20.b())) < Globals.Antialias_Threshold()) {
          return c22;
        }
      }
    }
  ...
  if (Globals.Sampling_Method() == 2) {
    ...
    Average += Antialias(c00, c01, c10, c11, u_0, v0_, (u_0 + u_2) / 2.0,
                         (v0_ + v2_) / 2.0, C) *
               0.25;

    Average += Antialias(c01, c02, c11, c12, (u_0 + u_2) / 2.0, v0_, u_2,
                         (v0_ + v2_) / 2.0, C) *
               0.25;

    Average += Antialias(c10, c11, c20, c21, u_0, (v0_ + v2_) / 2,
                         (u_0 + u_2) / 2.0, v2_, C) *
               0.25;

    Average += Antialias(c11, c12, c21, c22, (u_0 + u_2) / 2, (v0_ + v2_) / 2,
                         u_2, v2_, C) *
               0.25;
  } else {
    // Non adaptive...
    ...
    while (v < Globals.Antialias_Depth()) {
      ...
      while (u < Globals.Antialias_Depth()) {
        if (u || v) {
          Average +=
              TraceRay(u_2 - (u_2 - u_0) * u / Globals.Antialias_Depth(),
                       v2_ - (v2_ - v0_) * v / Globals.Antialias_Depth(), C);
        }
...
void DrawFlare(cBitmap Current_Bitmap, cBitmap LensFlare, cFColour lc, long u,
               long v) {
...
void cScene::Raytrace(void) {
  ...
  if (Globals.Verbose() > 0) {
    printf("Memory Consumed: %li bytes = %lg kb = %lg Mb\n",
           Globals.MemoryUsed(), (double)Globals.MemoryUsed() / 1024.0f,
           (double)Globals.MemoryUsed() / 1048576.0f);
    ...
    while (v < m_vImage) {
      ...
      while (u < m_uImage) {
        ...
        if (Globals.Antialias() && (u != 0) && (v != 0)) {
          tmp = Antialias(cIColour(&Current_Bitmap.Pixel((u - 1) +
                                                         ((v - 1) * m_uImage)))
                              .Convert(),
                          cIColour(&Current_Bitmap.Pixel((u - 0) +
                                                         ((v - 1) * m_uImage)))
                              .Convert(),
                          cIColour(&Current_Bitmap.Pixel((u - 1) +
                                                         ((v - 0) * m_uImage)))
                              .Convert(),
                          RayColour, du - (double)1.0 / m_uImage,
                          dv - (double)1.0 / m_vImage, du, dv,
                          m_Camera_List[Current_Camera])
                    .Convert();
        } else {
...
/////////////////////////////////////////////////////////////
    ...
    while (i < m_Number_Frames) {
      ...
      Camera_Hit = m_Camera_List[Current_Camera]->location().LorentzTransform(
          m_Camera_List[Current_Camera]->Velocity(),
          m_Frame_List[i]->Velocity());
      ...
      while (j < m_Frame_List[i]->Number_Lights()) {
        if (((oLight *)m_Frame_List[i]->Light_List(j))->LensFlare()) {
          ...
          Current_Ray = Current_Ray.LorentzTransform(
              m_Frame_List[i]->Velocity(),
              m_Camera_List[Current_Camera]->Velocity());
          ...
          if ((du > -0.5) && (du < 0.5) && (dv > -0.5) && (dv < 0.5)) {
            ...
            if (m_Frame_List[i]->Light_List(j)->no_shadow()) {
              ...
              ICurrent = CastRay(Current_Ray, m_Frame_List[i]->Velocity(),
                                 (coObject *)NULL);

              while ((ICurrent.From_Object() != NULL) &&
                     ((Camera_Hit.t() - ICurrent.Hit().t()) < abs(d))) {
                if (ICurrent.From_Object()->no_shadow()) {
                  // just call GetPigment() and let that function sort out
                  // whether to use a quick colour
                  Shadow_Hit = ICurrent.Hit().LorentzTransform(
                      m_Frame_List[i]->Velocity(),
                      ICurrent.From_Frame()->Velocity());
                  sc = ICurrent.From_Object()
                           ->GetPigment(Shadow_Hit)
                           .Doppler2(Current_Ray.doppler() *
                                     ICurrent.From_Object()
                                         ->Surface()
                                         ->P()
                                         .doppler()) *
                       Current_Ray.intensity();
            ...
            if ((ICurrent.From_Object() == NULL) ||
                ((Camera_Hit.t() - ICurrent.Hit().t()) > abs(d))) {
              // We have a (relatively) unobstructed path to the light_source,
              // so

              // FIXUP: This is dangerous messing around with Current_Ray...

              Current_Ray.Direction() = LorentzTransform(
                  Current_Ray.Direction(), m_Frame_List[i]->Velocity(),
                  m_Camera_List[Current_Camera]->Velocity());
              ...
              lc = m_Frame_List[i]
                       ...
                       ->GetPigment(cEvent(
                           o[0] + (d[0] / abs(d)), o[1] + (d[1] / abs(d)),
                           o[2] + (d[2] / abs(d)), Camera_Hit.t() - abs(d)))
              ...
              if (((oLight *)m_Frame_List[i]->Light_List(j))->fade_power() !=
                  0) {
                Current_Ray.intensity() *=
                    2 /
                    (1.0 + pow(abs(d) /
    ...
    if (Globals.HUD()) {
      ...
      // Bottom Left
      Current_Bitmap.Combine(u_l - 32, v_t - 32, u_l + 128, v_t + 128, 0, 0,
                             160, 160, m_HUD);
      // Top Left
      Current_Bitmap.Combine(u_l - 32, v_b - 128, u_l + 128, v_b + 32, 0, 160,
                             160, 320, m_HUD);
      // Bottom Right
      Current_Bitmap.Combine(u_r - 128, v_t - 32, u_r + 32, v_t + 128, 160, 0,
                             320, 160, m_HUD);
      // Top Right
      Current_Bitmap.Combine(u_r - 128, v_b - 128, u_r + 32, v_b + 32, 160, 160,
                             320, 320, m_HUD);
    }
...
void cScene::Load(char *FileName) {
  ...
  do {
    if (Globals.Test(T)) T = Globals.Load(T);
    ...
    switch (T->ID()) {
      ...
      case ID_camera:
        ...
        T = (*Current_Camera)
                ->Load(T, lift::make_vector<double>(0.0, 0.0, 0.0));
        Current_Camera++;
        break;
