...
class cMap : public RefCount, public cStack_Transform {
 ...
 protected:
  ...
  cIColour Evaluate_IColour(cEvent &o) {
    return Evaluate_FColour(o).Convert();
  };
 ...
 public:
  ...
  lift::vector<double, 3> _frequency, lift::vector<double, 3> _phase,
  lift::vector<double, 3> _drift,
  cwWarp * _warp
  )
  {
  ...
  */
  ...
  _inline double Scalar(cEvent o) {
    if (m_map_type == MT_NONE)
      return m_scalar;
    else
      return Evaluate_Scalar(o);
  };
  _inline lift::vector<double, 3> Vector(cEvent o) {
    if (m_map_type == MT_NONE)
      return m_vector;
    else
      return Evaluate_Vector(o);
  };
  _inline cFColour FColour(cEvent o) {
    if (m_map_type == MT_NONE)
      return m_fcolour;
    else
      return Evaluate_FColour(o);
  };
  _inline cIColour IColour(cEvent o) {
    if (m_map_type == MT_NONE)
      return m_fcolour.Convert();
    else
      return Evaluate_IColour(o);
  };
