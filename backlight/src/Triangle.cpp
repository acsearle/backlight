...
void coTriangle::init() { m_normal = lift::make_vector<double>(0, 0, 1); };
...
coTriangle::~coTriangle() { Globals.MemoryUsed() -= sizeof(coTriangle); };
...
double coTriangle::Interior(lift::vector<double, 3> o) { return 0; };

bool coTriangle::Bound_Intersection(cRay& RInitial, double t_min, double t_max,
                                    poObject poParent) {
  cRay nRay = RInitial;
  ciObject ICurrent;
  ...
  */
  ...
  // DAMN! This is all very well when it's the object casting the shadow on
  // itself, but
  // when its another coincident surface from a different shape, things get
  // ugly!

  // To rectify this, we resort to the quick-and-dirty method of not accepting
  // intersections too close to the surface of a ray-emitting object. After all,
  // if we can
  // see it...
  ...
  if (inner_product(o - m_bounded_by.o()) >
      (m_bounded_by.r() * m_bounded_by.r()) - EPSILON) {
    return false;
  }
...
cBound coTriangle::Bound(Array_Lights& L_S_L) {
  ...
  m_normal = (m_matGeometry.invert())
                 .TransformNormal(lift::make_vector<double>(0., 0., 1.));
...
ciObject coTriangle::Intersection(cRay& RInitial, double t_min, double t_max,
                                  poObject poParent) {
  ciObject IBest;
  lift::vector<double, 3> o, d;
  ...
  if (fabs(d[2]) > EPSILON) {
    ...
    if (x_test < -EPSILON || y_test < -EPSILON ||
        (x_test + y_test) > 1.0 + EPSILON)
      return IBest;
