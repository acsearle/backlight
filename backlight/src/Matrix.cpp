...
{
    lift::vector<double, 3> t;
    t[0] = (m_ary[1][1]*m_ary[2][2] - m_ary[2][1]*m_ary[1][2])*v[0] + (m_ary[0][2]*m_ary[2][1] - m_ary[2][2]*m_ary[0][1])*v[1] + (m_ary[0][1]*m_ary[1][2] - m_ary[1][1]*m_ary[0][2])*v[2];
    t[1] = (m_ary[1][2]*m_ary[2][0] - m_ary[2][2]*m_ary[1][0])*v[0] + (m_ary[0][0]*m_ary[2][2] - m_ary[2][0]*m_ary[0][2])*v[1] + (m_ary[0][2]*m_ary[1][0] - m_ary[1][2]*m_ary[0][0])*v[2];
    t[2] = (m_ary[1][0]*m_ary[2][1] - m_ary[2][0]*m_ary[1][1])*v[0] + (m_ary[0][1]*m_ary[2][0] - m_ary[2][1]*m_ary[0][0])*v[1] + (m_ary[0][0]*m_ary[1][1] - m_ary[1][0]*m_ary[0][1])*v[2];
    return t;
}
...
cMatrix cMatrix::invert(void) 
{
    ...
    // FIXUP: This is not really the "end."  All it shows is that there is a null kernel in
    //        the transform... we could determine the eigenvalues and eigenvectors of the 
    //        non-null kernel and use them to constuct an inverse matrix.  But that seems like too
    //        much work!
    double deter = det();
    ...
    t.m_ary[0][0] = (m_ary[1][1]*m_ary[2][2] - m_ary[1][2]*m_ary[2][1]) / deter;
    t.m_ary[0][1] = (m_ary[0][2]*m_ary[2][1] - m_ary[0][1]*m_ary[2][2]) / deter;
    t.m_ary[0][2] = (m_ary[0][1]*m_ary[1][2] - m_ary[0][2]*m_ary[1][1]) / deter;

    t.m_ary[1][0] = (m_ary[1][2]*m_ary[2][0] - m_ary[1][0]*m_ary[2][2]) / deter;
    t.m_ary[1][1] = (m_ary[0][0]*m_ary[2][2] - m_ary[0][2]*m_ary[2][0]) / deter;
    t.m_ary[1][2] = (m_ary[0][2]*m_ary[1][0] - m_ary[0][0]*m_ary[1][2]) / deter;

    t.m_ary[2][0] = (m_ary[1][0]*m_ary[2][1] - m_ary[1][1]*m_ary[2][0]) / deter;
    t.m_ary[2][1] = (m_ary[0][1]*m_ary[2][0] - m_ary[0][0]*m_ary[2][1]) / deter;
    t.m_ary[2][2] = (m_ary[0][0]*m_ary[1][1] - m_ary[0][1]*m_ary[1][0]) / deter;
  ...
  switch(m_type)
  {
    ...
    case TT_SCALE:
      T = ::Load(v,T);
        ...
        Initialize(TT_SCALE, 1/v[0],0,0,  0,1/v[1],0,  0,0,1/v[2], 0,0,0);
        break;
    ...
    case TT_ROTATE: 
      T = ::Load(v,T);
        ...
        Rx.Initialize( TT_ROTATE,  1,0,0,                    0,cos(v[0]),-sin(v[0]),  0,sin(v[0]),cos(v[0]), 0,0,0 );
        Ry.Initialize( TT_ROTATE,  cos(v[1]),0,-sin(v[1]), 0,1,0,                     sin(v[1]),0,cos(v[1]), 0,0,0 );
        Rz.Initialize( TT_ROTATE,  cos(v[2]),-sin(v[2]),0, sin(v[2]),cos(v[2]),0,   0,0,1,                   0,0,0 );
