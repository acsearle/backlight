...
namespace lift
{
  ...
  public:
    ...
    template<typename U> explicit vector(const vector<U, N>& that) 
    { 
      std::uninitialized_copy(that.begin(), that.end(), begin()); 
    }

#pragma warning (disable : 4267)
    template<typename U> explicit vector(const U& that) 
    { 
      iterator i(begin());
      try
      {
        for (U j(that); i != end(); ++i, ++j)
          new (const_cast<void*>(reinterpret_cast<const void*>(i))) value_type(*j);
      }
      catch (...)
      {
        for (reverse_iterator j(i); j != rend(); ++j)
...
#pragma warning (default : 4267)
    ...
    template<typename U> vector& operator=(const vector<U, N>& that) 
    { 
      std::copy(that.begin(), that.end(), begin()); 
      return *this; 
    }

    template<typename U> void swap(vector<U, N>& that) 
    { 
    ...
    template<typename U> vector<T, N>& operator+=(const vector<U, N>& that) 
    {
      for_each(make_zip_iterator(make_tuple(begin(), that.begin())), make_zip_iterator(make_tuple(end(), that.end())), tuple_assigned_plus<T, U>());
      return *this; 
    }

    template<typename U> vector<T, N>& operator-=(const vector<U, N>& that) 
    { 
      for_each(make_zip_iterator(make_tuple(begin(), that.begin())), make_zip_iterator(make_tuple(end(), that.end())), tuple_assigned_minus<T, U>());
      return *this; 
    }
  ...
  template<typename T, size_t N> struct scalar<vector<T, N> >
  {
  ...
  {
    return vector<T, N>(make_transform_iterator(make_zip_iterator(make_tuple(left.begin(), right.begin())), tuple_plus<T>()));
  }

  template<typename T, size_t N> vector<T, N> operator-(const vector<T, N>& left, const vector<T, N>& right)
  {
    return vector<T, N>(make_transform_iterator(make_zip_iterator(make_tuple(left.begin(), right.begin())), tuple_minus<T>()));
  }
  ...
  template<typename T, size_t N> T inner_product(const vector<T, N>& left)
  {
    return inner_product(left, left);
  }

  template<typename T, size_t N> T abs(const vector<T, N>& x)
  {
  ...
  template<typename T, size_t N, typename U> bool operator==(const vector<T, N>& left, const vector<U, N>& right)
  {
  ...
  template<typename T, size_t M, typename U, size_t N> bool operator<(const vector<T, M>& left, const vector<U, N>& right)
  {
  ...
  template<size_t N, typename T> ostream& operator<<(ostream& left, const vector<T, N>& right)
  {
    left << "(";
...
#define LIFT_MAKE_VECTOR(Z, N, DATA)\
  template<typename T> vector<T, N> make_vector(BOOST_PP_ENUM_PARAMS(N, const T& a))\
  {\
