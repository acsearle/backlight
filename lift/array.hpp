...
namespace lift
{
  ...
  public:
    ...
    template<typename U> explicit array(const array<U, N>& that) 
    { 
      std::uninitialized_copy(that.begin(), that.end(), begin()); 
    }

    template<typename U> explicit array(const U& that) 
    { 
      iterator i(begin());
      try
      {
    ...
    template<typename U> array& operator=(const array<U, N>& that) 
    { 
      copy(that.begin(), that.end(), begin()); 
      return *this; 
    }

    template<typename U> void swap(array<U, N>& that) 
    { 
  ...
  template<typename T, size_t N, typename U> bool operator==(const array<T, N>& left, const array<U, N>& right)
  {
  ...
  template<typename T, size_t M, typename U, size_t N> bool operator<(const array<T, M>& left, const array<U, N>& right)
  {
    return lexicographical_compare(left.begin(), left.end(), right.begin(), right.end());
  }

  template<size_t N, typename T> std::ostream& operator<<(std::ostream& left, const array<T, N>& right)
  {
    left << "(";
...
#define LIFT_MAKE_ARRAY(Z, N, DATA)\
  template<typename T> array<T, N> make_array(BOOST_PP_ENUM_PARAMS(N, const T& a))\
  {\
